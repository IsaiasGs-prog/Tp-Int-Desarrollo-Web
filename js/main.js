...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function updateNavLinks() {
        ...
        if (loginLogoutLink) {
            if (accessToken) {
                ...
                loginLogoutLink.querySelector('a').textContent = `Cerrar Sesión (${username})`;
        ...
        if (loginLogoutLinkSidebar) {
            if (accessToken) {
                const username = currentUser ? currentUser.username : 'Usuario';
                loginLogoutLinkSidebar.querySelector('a').textContent = `Cerrar Sesión (${username})`;
        ...
        if (adminNavLink) {
            if (isAdmin) {
                adminNavLink.classList.remove('hidden-admin');
            } else {
                adminNavLink.classList.add('hidden-admin');
            }
        }
        
        if (adminNavLinkSidebar) {
            if (isAdmin) {
                adminNavLinkSidebar.classList.remove('hidden-admin');
            } else {
                adminNavLinkSidebar.classList.add('hidden-admin');
            }
        }
    }
    ...
    if (window.location.pathname.includes('salones-crud.html')) {
        ...
        if (!isAdmin) { 
            alert('Acceso permitido, pero no tienes privilegios de administrador para ver esta sección.');
            window.location.href = 'index.html'; 
            return;
        }

        document.querySelectorAll('.hidden-admin').forEach(element => {
        ...
        const currentUserCrud = JSON.parse(sessionStorage.getItem('currentUser'));
        if (currentUserCrud) {
            console.log(`Bienvenido, ${currentUserCrud.firstName}! Has iniciado sesión como administrador en el CRUD.`);
        }
    } 
});
